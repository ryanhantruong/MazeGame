...
import hanscott.cs301.cs.wm.edu.amazebyhanscott.GeneratingActivity;
...
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.Factory;
...
public class MazeController implements Order {
	...
	private GeneratingActivity generate;
	...
	public void init() {
		...
		//System.out.println("set to state_title");
		rset = new RangeSet();
		//panel.initBufferImage() ;
		//addView(new MazeView(this)) ;
		//notifyViewerRedraw() ;
	...
	public void switchToPlayingScreen() {
		writeLog("entered switchToPlayingScreen");
		...
		writeLog("about to initiate firstDrawer");
		FirstPersonDrawer firstDrawer = new FirstPersonDrawer(Constants.VIEW_WIDTH,Constants.VIEW_HEIGHT, Constants.MAP_UNIT,
				Constants.STEP_SIZE, seencells, mazeConfig.getRootnode()) ;
		writeLog("pass initiate firstDrawer");
		firstDrawer.setController(this);		
		addView(firstDrawer);
				
		// order of registration matters, code executed in order of appearance!
		MapDrawer mapDrawer = new MapDrawer(Constants.VIEW_WIDTH,Constants.VIEW_HEIGHT,Constants.MAP_UNIT,
				Constants.STEP_SIZE, seencells, 20, this) ;
		addView(mapDrawer);
				
		notifyViewerRedraw() ;
		writeLog("about to exit switchToPlayingScreen");
	}
	...
	protected void switchToFinishScreen(float energyConsumed, int pathLength, boolean win) {
		...
		notifyViewerRedraw(energyConsumed,pathLength, win) ;*/
		this.over = true;
		this.result = win;
		this.energyConsumed = energyConsumed;
		this.pathLength = pathLength;
		writeLog("energy consumed"+energyConsumed);
		writeLog("path length"+pathLength );
	...
	/**
	 * Switches to finish screen, old method with no robot parameters 
	 * This transition is only possible from the playing screen
	 * by making a forward or backward move through the exit outside
	 * of the maze.
	 */
	...
	public void notifyViewerRedraw() {
		...
		while (it.hasNext())
		{
			writeLog("in while loop of notifyViewerRedraw");
			// for android. use invalidate() or postInvalidate(),
			Viewer v = it.next() ;
			Object g = panel.getBufferGraphics() ;
			// viewers draw on the buffer graphics
			if (null == g) {
				writeLog("Maze.notifierViewerRedraw: can't get graphics object to draw on, skipping redraw operation") ;
			}
			else {
				/*if (state!=StateGUI.STATE_PLAY){ v.redraw(g, state, 0,0,true);}
				else {
				v.redraw(g, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle) ;}*/
				writeLog("about to call redraw of a viewer");
				v.redraw(g, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle) ;
				writeLog("pass redraw of a viewer");
			}
		}
		// update the screen with the buffer graphics
		writeLog("about to call on panel.update()");
		//panel.update() ;
		play.reset();
		...
		writeLog("passed call on panel.update()");

	}

	public void setPlayActivity(PlayActivity play){ this.play=play;}
	public void setGeneratingActivity(GeneratingActivity generate){ this.generate=generate;}
	...
	public RobotDriver getDriver() {return driver;}
	...
	*/
	...
	public void setDriver(String str){
		...
		if(str.equals("Manual")){
			...
			driver = new Wizard();
			System.out.print(robot==null);
			driver.setRobot(robot);
		}
		else if(str.equals("WallFollower")){
	...
	public void passManualDriver(RobotDriver manualDriver){
		driver = manualDriver;
	}
	...
	public void deliver(MazeConfiguration config) {
		// we has initiated a mazeConfig object for this class in the init() method, 
		// to work well with our robot that uses this config, lets transfer all critical info from the config parameter to the mazeConfig we initiated
		mazeConfig.setHeight(config.getHeight());
		mazeConfig.setWidth(config.getWidth());
		mazeConfig.setMazecells(config.getMazecells());	
		mazeConfig.setMazedists(config.getMazedists());
		mazeConfig.setRootnode(config.getRootnode());
		int[] pos = config.getStartingPosition();
		mazeConfig.setStartingPosition(pos[0], pos[1]);
		...
		try {
			//writeLog("about to tell solver drive 2 exit");
			//driver.drive2Exit();
			//writeLog("passed tell solver drive 2 exit");
		} catch (Exception e) {
	...
	public void updateProgress(int percentage) {
		if (percentdone < percentage && percentage <= 100) {
			while (percentdone<percentage){
				percentdone++;
				generate.updateProgress(percentage);}
