...
public class MapDrawer extends DefaultViewer {
	...
	public void redraw(Object gc, StateGUI state, int px, int py,
		...
		/*if (state != StateGUI.STATE_PLAY)
			return ;*/
		writeLog("in MapDrawer - started redraw");
		if (null != controller) {
			if (controller.isInMapMode()) {
		...
		writeLog("in MapDrawer - done w redraw");
	}
	 ...
	 */
	public void draw_map(Object gc, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		// dimensions of the maze
		final int mazew = mazeConfig.getWidth() ;
		final int mazeh = mazeConfig.getHeight() ;
		
		//gc.setColor(Color.white);
		MazePanel.setGraphicsColor(MazePanel.getColor("white"),gc);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				MazePanel.setGraphicsColor(seencells.hasWall(x,y, CardinalDirection.North) ? MazePanel.getColor("white"): MazePanel.getColor("gray"),gc);
				if ((seencells.hasWall(x,y, CardinalDirection.North) || showMaze) && theCondition)
					//gc.drawLine(nx1, ny1, nx2, ny1)
					MazePanel.drawGraphicsLine(nx1, ny1, nx2, ny1, gc);
				...
				if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
					...
					MazePanel.drawGraphicsLine(nx1, ny1, nx1, ny2, gc);
			}
	...
	public void draw_currentlocation(Object gc, int view_dx, int view_dy) {
		...
		MazePanel.setGraphicsColor(MazePanel.getColor("red"),gc);
		...
		//MazePanel.fillGraphicsOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz, gc);
		MazePanel.fillGraphicsOval(ctrx-cirsiz/2, ctry-cirsiz/2, ctrx+cirsiz/2, ctry+cirsiz/2, gc);
		...
		MazePanel.drawGraphicsLine(ctrx, ctry, aptx, apty, gc);
		//gc.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		MazePanel.drawGraphicsLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy, gc);
		//gc.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
		MazePanel.drawGraphicsLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy, gc);
	}
	 ...
	 * @param offy
	 */
	public void draw_solution(Object gc, int offx, int offy, int px, int py) {
		...
		MazePanel.setGraphicsColor(MazePanel.getColor("yellow"),gc);
		...
		while (distance > 1) {
			...
			MazePanel.drawGraphicsLine(nx1, ny1, nx2, ny2, gc);
