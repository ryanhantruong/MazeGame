...
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.BSPBranch;
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.BSPLeaf;
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.BSPNode;
...
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.MazeBuilder;
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.Seg;
import hanscott.cs301.cs.wm.edu.amazebyhanscott.generation.Wall;

import java.util.ArrayList;

import hanscott.cs301.cs.wm.edu.amazebyhanscott.falstad.MazePanel.*;
...
public class FirstPersonDrawer extends DefaultViewer {
	...
	private Object gc ; // the graphics object for the buffer image this class draws on
	...
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells seencells, BSPNode bsp_root) {
		...
		writeLog("instantiate FirstPersonDrawer");
	}
	 ...
	 */
	@Override
	public void redraw(Object gc, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		// if notified by model that state has changed
		// Query model for parameters
		//dbg("viewer.redraw called");

		writeLog("in FirstPerson - started redraw");
		...
		panel.setGraphicsColor(panel.getColor("white"),gc);
		...
		writeLog("in FirstPerson - done with redraw");
	}
	 ...
	 * @param gc
	 */
	private void drawBackground(Object gc) {
		//gc.setColor(Color.black);
		//gc.fillRect(0, 0, view_width, view_height/2);
		//gc.setColor(Color.darkGray);
		//gc.fillRect(0, view_height/2, view_width, view_height/2);

//		writeLog("before setting background");

		panel.setGraphicsColor(MazePanel.getColor("black"),gc);
		panel.fillGraphicsRect(0, 0, view_width, view_height/2,gc);
		panel.setGraphicsColor(MazePanel.getColor("darkGray"),gc);
		panel.fillGraphicsRect(0, view_height/2, view_width, view_height,gc);

	//	writeLog("after setting background");
	}
	...
	private void drawAllVisibleSectors(BSPNode nn) {
		...
		//writeLog("before drawing visible vectors");
		...
		//writeLog("after drawing visible vectors");
	}
	...
	private void drawSegment(Seg seg) {
		...
		int x1i = x1;
		int xd = x2-x1;
		panel.setGraphicsColor(seg.getColor(),gc);
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			panel.fillGraphicsPolygon(xps, yps, 4, gc);
