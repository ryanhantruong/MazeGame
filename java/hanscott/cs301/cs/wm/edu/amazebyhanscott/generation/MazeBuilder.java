...
import hanscott.cs301.cs.wm.edu.amazebyhanscott.falstad.Constants;
...
public class MazeBuilder implements Runnable {
	...
	// lets change cells and distance to public so we can use from different package? maybe lets keep it protected
	// but make methods to return it references
	...
	// ADDED TO SET STARTING POSITION
	MazeConfiguration mazeConfig;
	public void setMazeConfig(MazeConfiguration mazeConfig){
		this.mazeConfig = mazeConfig;
		System.out.println("builder recieved config");
	}
	...
	public MazeBuilder(boolean deterministic){
		...
		{
			// Set the seed value equal to 1 before initialize random variable
	...
	public void run() {
		...
		try {
			...
			if (mazeConfig.getMazecells()==null){
				System.out.println("config cells test in maze builder returns null");
			}
	...
	protected void generate() {
		...
		final int[] pos;
		if (mazeConfig !=null){
			pos = mazeConfig.getStartingPosition();}
		else {
			pos = dists.getStartPosition();
		}
		...
		//System.out.println("Maze generated with start x,y: "+startx+","+starty+" exit x,y: "+remote[0]+","+remote[1]);
	...
	protected void generatePathways() {
		// Chose random cell along top row (x,0)
		int x = random.nextIntWithinInterval(0, width-1);
		int y = 0; 
		final int firstx = x; // this cell is the starting position
		final int firsty = y;
		
		// arrays of direction (looks like original direction for the whole maze board)
		CardinalDirection[][] origcds = new CardinalDirection[width][height]; 
		// Chose east direction
		CardinalDirection cd = CardinalDirection.East;
		CardinalDirection origcd = cd; // direction of the starting cell is east
		
		// set starting cell  as visited
		cells.setCellAsVisited(x, y);
		// make a Wall instance for the cell, in cd direction which is east
		Wall wall = new Wall(x, y, cd);
		while (true) { 
			// what does setWall do?
			wall.setWall(x, y, cd); // this step is not necessary in the first loop maybe for later loop? YES
			// we used this same Wall instance for the cell of new iteration
			if (!cells.canGo(wall)) { // if we cant go , why pass wall object? 
				// rotate direction, for first loop we rotate from east to south
				cd = cd.rotateClockwise(); //rotate direction
				if (origcd == cd) {	// if still same direction, didnt rotate	
					if (x == firstx && y == firsty) // if still the same cell
						break; 			// get out of loop
					int[] dxy = origcds[x][y].getDirection(); // get direction for cell x,y
					x -= dxy[0]; // pass reference to cell in this rection
					y -= dxy[1];
					if (null == origcds[x][y]) { // if current x y cell does not have direction yet
						// Happens at starting position
						assert (x == firstx && y == firsty) : "catching null elsewhere than starting position" ;
						cd = cd.randomDirection() ; // chose random direction
					}
					else // get direction defined in current cell
						cd = origcds[x][y] ;
					cd = cd.rotateClockwise(); // rotate direction
					origcd = cd; // save as direction of last iteration
				}
				
			} else {// if we can go (go in east for first loop)
				cells.deleteWall(wall); // delete the wall 
				int[] dxy = cd.getDirection(); // get predefined direction in cd (east in first loop)
				x += dxy[0]; // update to this cell position
				y += dxy[1];
				cells.setCellAsVisited(x, y); // set this cell as visited 
				origcds[x][y] = cd; // update dir arrays with direction cd (with operation how get us here)
				cd = cd.randomDirection(); // get a random direction
				origcd = cd; // save this as direction of last iteration
			}
		}
	}
